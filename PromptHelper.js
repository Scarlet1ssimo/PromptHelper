...
Execute this protocol now. Prioritize accuracy over speed. Think deeply, research thoroughly, interact intelligently.`
        ...
        function injectStyles() {
            GM_addStyle(`
                ...
                #prompt-helper-container.ph-retracted {
                    transform: translateX(calc(100% - 12px)) !important;
                    opacity: 0.5 !important;
                }
                #prompt-helper-container.ph-retracted:hover {
                    transform: translateX(0) !important;
                    opacity: 1 !important;
                }
                
                /* Increase trigger distance (invisible aura) */
                #prompt-helper-container::before {
                    content: '' !important;
                ...
                /* 快速应用按钮（动态） */
                #prompt-helper-container .ph-quick-btn {
                    width: 100% !important;
                ...
                #prompt-helper-container .ph-quick-btn:hover { filter: brightness(0.95) !important; }
        ...
        function buildUI() {
            ...
            D.quickBtnContainer = create('div', 'prompt-helper-quick-container');
            ...
            const containerNodes = [D.toggleButton, D.quickBtnContainer, D.contentPanel];
                            ...
                            setTimeout(() => {
                                ...
                                const protect = () => {
                                    if (protectionCount < 20) {
                                        const currentHtml = inputElement.innerHTML; const currentText = inputElement.textContent || inputElement.innerText;
        ...
        function buildAndInit() {
            ...
            const updateUI = () => {
                ...
                D.settingsButton.title = t.settingsTitle;
                // D.quickApplyButton.textContent = t.quickApplyBtn; // REMOVED
            ...
            updateUI();
            renderQuickButtons(); // Initial render

            // 交互
            D.toggleButton.addEventListener('click', () => {
                D.contentPanel.classList.remove('hidden');
                container.classList.remove('ph-retracted'); // Ensure expanded when opening
            });
            D.collapseButton.addEventListener('click', () => {
                D.contentPanel.classList.add('hidden');
                // Check if mouse is currently over the container?
                // The mouseleave event handles the retraction, but if we click close while hovering, it stays expanded until we leave.
                // That is correct behavior (expand on hover).
            });

            // Auto-hide Logic
            container.addEventListener('mouseenter', () => {
                container.classList.remove('ph-retracted');
            });
            container.addEventListener('mouseleave', () => {
                if (D.contentPanel.classList.contains('hidden')) {
                    container.classList.add('ph-retracted');
                }
            });

            // Init retracted state if panel is hidden (it is by default)
            setTimeout(() => {
                if (D.contentPanel.classList.contains('hidden')) {
                    container.classList.add('ph-retracted');
                }
            }, 1000); // Delay a bit on load
            ...
            function renderQuickButtons() {
                D.quickBtnContainer.textContent = '';

                // Order: Default, then site rules? Or just existing list?
                // Requirement: "Give all saved templates an Apply button"
                // Let's iterate through `prompts`

                // 1. Default Template
                if (prompts[DEFAULT_TEMPLATE_ID]) {
                    addQuickButton(DEFAULT_TEMPLATE_ID, prompts[DEFAULT_TEMPLATE_ID].name);
                }

                // 2. Others
                for (const id in prompts) {
                    if (id === DEFAULT_TEMPLATE_ID) continue;
                    addQuickButton(id, prompts[id].name);
                }
            }

            function addQuickButton(id, name) {
                const btn = document.createElement('button');
                btn.className = 'ph-quick-btn';
                btn.textContent = name;
                btn.title = name;
                btn.addEventListener('click', (e) => {
                    e.stopPropagation(); // prevent interfering with container hover maybe?
                    applyTemplateById(id);
                });
                D.quickBtnContainer.appendChild(btn);
            }

            function applyTemplateById(id) {
                const tpl = prompts[id]?.template || '';
                const res = getFinalFromChatByTemplateStr(tpl);
                if (res.error === 'no_template') { alert(translations[currentLang].alertTemplateError); return; }
                if (res.error === 'no_input') { alert(translations[currentLang].alertSubmitError); return; }
                if (res.error === 'no_user_input') { alert(translations[currentLang].alertNoUserInput); return; }
                applyPromptToChat(res.inputEl, res.final);
                // Don't hide for these buttons? Or should we? Default behavior was to hide panel if using inner apply.
                // Quick apply button usually keeps panel hidden.
                // If the panel is open, we can close it? 
                // The requirement is "mouse leave to retract". 
                // If I click a button, I am likely hovering.
            }
